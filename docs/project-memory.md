...
- 一项 `WIKI_INGEST` 因分析引用重复片段 ID 达到 3 次尝试后失败。来源快照继续保留原始分析，页面引用去重，真实片段与时间范围仍严格校验。另有融合模型未提交有效变更集的独立失败，后续同类回合将按有限预算自动重试。当前运行中的后端/Worker 需要重启后加载修复，已经终止的历史任务需由用户在任务中心手动重试；诊断期间未修改私人 Wiki 或任务。
