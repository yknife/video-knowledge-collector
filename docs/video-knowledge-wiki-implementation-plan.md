...
版本：1.4 · 制定/更新日期：2026-09-25 · 状态：阶段 0—7 已通过；阶段 8 待验收（真实桌面时间回看点击）。
...
| 1 | Wiki 初始化与存储 | 0 | 已通过（见 `docs/wiki-stage-1.md`） |
| 2 | 单视频来源与知识页 | 1 | 已通过（见 `docs/wiki-stage-2.md`） |
| 3 | 自动入库、补录与恢复 | 2 | 已通过（见 `docs/wiki-stage-3.md`） |
| 4 | 桌面知识库浏览与检索 | 3 | 已通过（见 `docs/wiki-stage-4.md`） |
| 5 | 跨视频主题与实体融合 | 4 | 已通过（见 `docs/wiki-stage-5.md`） |
...
- [x] 空目录初始化成功；再次初始化不覆盖用户文件，未知非空目录不会被直接重置。
- [x] 中文标题、Windows 路径、超长标题和同名标题可处理；`../`、绝对路径、符号链接/junction 越界均被拒绝。
- [x] 提交一组页面后 revision、hash、index、log 对应同一 commit_id。
- [x] 在发布中途注入故障，重启后应用只能读取完整旧版或完整新版，无半成品。
- [x] 模拟旧 Worker 租约过期，旧 fencing token 写入被拒绝。
...
- [x] 样本 A 生成来源快照与视频页，摘要、章节、知识点、问答完整可读。
- [x] 每条结构化引用验证通过，引用数量及时间范围与原分析一致。
- [x] 重复入库 A 不新增重复页面、来源包或提交日志。
- [x] 入库 F 后视频页稳定 ID 不变、修订增加、A 的历史来源仍可访问。
- [x] E 的降级段落明显标识；不伪装为验证后的知识。
- [x] L1/L2 各自有页，场次目录关联正确，引用跳转到对应分段。
...
- [x] 自动入库开启时，一次分析完成恰好产生一次逻辑入库；桌面与飞书来源一致。
- [x] 在分析事务提交后、排队消费前杀进程，重启仍能生成入库任务。
- [x] 关闭自动入库后不新建自动任务；手动入库可用，已排队任务的处理策略在设置中说明。
- [x] 重复事件、重复点击、多个消费者不重复发布；新版先完成、旧版后完成不会回退页面。
- [x] 入库失败不改变分析成功状态；单独重试只处理 Wiki，不重做下载、转写和分析。
- [x] 补录预览能区分新增、已同步、版本更新、无分析、降级待复核；取消后已完成项保持有效。
- [x] 文件提交后数据库确认失败可恢复；一次逻辑提交只留一条可见日志。
...
- [x] 一个 Hermes 启动入口即可访问知识库；没有额外要求启动的服务。
- [x] A/D 中文关键词能命中正确页面；目录、标题和正文显示正常。
- [x] 点 A 中任一时间引用打开正确媒体，播放器跳转误差目标不超过 1 秒，受媒体可寻址性限制的情况记录证据。
- [x] 无结果、未初始化、长正文、缺失媒体、失效引用、任务失败都有明确界面。
- [x] 清除测试环境的搜索投影后可重建，结果数量与内容不丢失；重建不改写正文。
- [x] Markdown 中脚本、危险 URL、任意本地文件引用不能执行或绕过媒体访问边界。
...
- [x] A/B 共用同一主题/实体页面，别名不产生重复页；D 不因无关同词被错误合并。
- [x] C 的冲突观点与 A/B 的观点并列展示，均有时间、来源和争议标记；不因日期较新就判其正确。
- [x] 每项综合结论有可解析证据；推断、作者观点和材料中的事实明确区分。
- [x] E 的降级结果不自动成为高可信主题断言。
- [x] 两个任务更新同一页面产生 revision 冲突时重新合并或待复核，不能丢失任一方证据。
- [x] 相同输入复跑不因模型措辞变化反复生成修订；编译器版本升级可显式重编译。
- [x] 注入式 Transcript、非法变更路径、模型虚构片段 ID 被拒绝且原页面保持完整。
- [x] 真实 Hermes 模型完成 A/B/C 融合，逐条人工复核全部引用与预期冲突，记录模型、耗时、调用量和结果差异。
- [x] 真实 ingest 记录证明技能已加载并进入执行上下文，SCHEMA/index/近期 log 在知识处理前被读取，页面变更通过受控工具提交。
- [x] 模拟技能缺失/禁用/hash 不匹配/加载失败，融合明确失败或待处理，不能用普通 Prompt 静默替代；原来源包与视频页保持可用。
- [x] 新来源自动融合及旧基础视频页融合补录均调用同一技能入口，重复任务不重复发布。
...
- [x] 对 10 个预设问题测试：单视频事实 3 个、跨视频综合 3 个、冲突 2 个、证据不足 2 个。
- [x] 8 个可回答问题均满足预设关键点，引用存在且支持对应结论；2 个证据不足问题明确说明不足，不捏造。
- [x] 跨视频回答覆盖相关来源；冲突问题呈现分歧而非强行给出统一答案。
- [x] 未点击保存不新增 Wiki 页；同一答案重复保存不重复建页。
- [x] 保存的研究答案保留底层视频证据，不能把模型自己的旧答案当作新增独立佐证。
- [x] 已保存 query 页面能被后续搜索和问答找到，引用可回看；检索范围限制在当前 Wiki/profile。
- [x] 至少一次真实 query 的加载、规范读取、检索与证据调用记录完整；有无“保存”操作的权限和产物符合预期，不能以单次裸模型请求替代该流程。
...
- [x] 注入预设结构错误，结构巡检全部发现；语义问题按预设样本人工复核，不承诺开放语料中 100% 检出。
- [x] 修复索引和断链建议后重复巡检结果稳定，原始快照不变。
- [x] 编辑生成页面后，后续同步检测到外部修改并给出差异；`notes/` 内容不被覆盖。
- [x] 回退页面生成新的修订且保留审计，当前索引、反向链接同步更新。
- [x] 撤回 B 后相关页面不再把 B 作为有效支撑；历史快照保留并标明撤回，不删除视频。
- [x] 删除媒体与删除知识为不同操作；修改 Wiki 规范可预览受影响内容并选择重编译。
- [x] 真实 lint 的技能加载与页面读取证据可追踪，发现样本中的预设语义冲突；仅运行结构检查不能标记语义巡检验收通过。
...
- [ ] 完成一次本地媒体→分析→自动 Wiki 入库→主题更新→提问→点击时间回看的真实闭环。自动媒体到可解析时间路由已通过；真实桌面播放器点击待验收。
- [x] 至少 10 个历史视频和一组直播分段完成补录；缺少真实资料时由固定样本补足并明确标注，两者分别报告。
- [x] 不下载、不重新转写、不重做已有分析；重复补录不新增重复页。
- [x] 注入模型超时、磁盘空间不足、进程退出、提交冲突、索引写入失败均可诊断与恢复，已提交内容不丢失。
- [x] 并发两项融合不丢更新；旧租约写入被拒绝；数据库与文件一致性检查通过。
- [x] 在记录配置的测试机器上，1,000 个页面的本地搜索暖缓存 p95 目标 < 1 秒，读取页面 p95 < 500 ms（不含模型和播放器）；记录至少 30 次请求的方法与结果，未达标说明瓶颈。
- [x] 从备份恢复到新测试目录，可阅读页面、重建索引并追溯来源；原媒体未随备份迁移时明确提示。
- [x] 现有采集、直播、转写、分析、飞书通知及桌面入口回归通过。
- [x] 使用文档说明自动入库、补录、同步失败、争议、人工编辑、撤回来源、备份及模型费用。
- [x] 使用固定版本 llm-wiki 完成真实 ingest/query/lint 三条链路，审计可关联到页面修订、答案和巡检报告；演练技能加载失败及受控升级，不静默绕过技能。
